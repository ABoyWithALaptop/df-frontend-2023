...
function rangePagi(current, sibling, totalPage) {
  ...
  if (current <= 1 + sibling + 1) {
    ...
    const temp = Array.from({ length: sibling * 2 + 2 }).map((_, index) => 1 + index + 1)
    range.push(...temp)
    range.push("...")
    range.push(totalPage)
    return range
  }
  else if (lastPageInCurrentInteractRange >= totalPage - 1) {
    const temp = Array.from({ length: sibling * 2 + 2 }).map((_, index) => totalPage - index - 1).reverse()
    range.push("...")
    range.push(...temp)
    range.push(totalPage)
    return range
    ...
    range.push("...");
    const temp = Array.from({ length: sibling + 1 }).map((_, index) => current - sibling + index)
  ...
  if (lastPageInCurrentInteractRange < totalPage - 1) {
    const temp = Array.from({ length: sibling }).map((_, index) => current + index + 1)
    ...
    const temp = Array.from({ length: pageLeft }).map((_, index) => totalPage - index).reverse()
    range.push(...temp)
    // range.push(totalPage)
...
function PaginationBar({ totalPage, siblingCount = 1, currentPage, setCurrentPage }) {
  // const totalPage = 10
  const DOTS = "..."
